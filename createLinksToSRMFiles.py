...
def checkArguments(args):
    if not args.output_dir:
        ...
        sys.exit(1)
    if not only1([args.pattern, args.sampleList]):
        LOGGER.critical("Please provider either --sampleList OR --pattern")
        sys.exit(1)
    if args.pattern:
        if args.scope:
            if ":" in args.pattern:
...
def main(args):
    ...
    if args.sampleList:
        ...
        for did in ruc.list_dids(
            scope=scopename, filters={"name": pattern}, type="container", long=False
        ):
            samples.append(did)
    if not samples:
        sys.exit(
